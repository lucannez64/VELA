...
//! primary keys and as sled key fragments. Nothing checked their length or
//! charset: the 414 hyper returns on a very long URI is an accident of the HTTP
//! stack, not a decision this service made, and it stops applying the moment a
//! request arrives over HTTP/2 or HTTP/3, where there is no request line to
//! overflow (audit, server hardening).
//!
...
/// Longest identifier we will accept. Ours are under 20 characters; this leaves
/// room for a client with a different scheme without letting anyone stream a
/// megabyte into a key.
...
/// everything non-ASCII so two ids that render identically cannot be distinct.
...
///
/// `what` names the field in the error, which is safe to echo back: the caller
...
pub fn validate_id<'a>(what: &str, id: &'a str) -> Result<&'a str> {
    ...
    // the shape a path-traversal attempt takes if an id ever reaches a
    // filesystem.
...
mod tests {
    ...
    fn rejects_an_unbounded_id() {
        assert!(validate_id("chunk_id", &"a".repeat(MAX_ID_LEN + 1)).is_err());
        assert!(
            validate_id("chunk_id", &"a".repeat(MAX_ID_LEN)).is_ok(),
