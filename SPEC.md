...
VELA v2.0 abandons server-side passwords and password-derived server authentication. It is a multi-device, hardware-bound, hybrid post-quantum secure vault. Devices authenticate to the server by signing single-use challenges with a hybrid ML-DSA-87 + Ed25519 identity key. Clients may still protect local vault material with an optional device password fallback for systems that do not provide usable biometric hardware.
...
- **Hybrid Signature Authorization:** The server authenticates devices via ML-DSA-87 + Ed25519 signatures over single-use challenges. Both signature components must verify, providing post-quantum and classical defense in depth.
...
│  │                 (Rust crypto core · WASM build)                  │  │
│  │  - Hybrid ML-DSA-87 + Ed25519 device signatures                 │  │
...
│  │ - Hybrid challenge signature verifier                            │  │
...
| :--- | :--- | :--- |
| **Identity/Auth Signature** | **Hybrid: ML-DSA-87 + Ed25519** | Both signatures must verify over a domain-separated, single-use server challenge. ML-DSA is standardized by NIST FIPS 204. |
...
| `device_identity_key` | `"vela device identity v1"` | Local protection context for device identity material |
| `identity_signing_key` | `"vela identity signing v1"` | Local protection context for hybrid ML-DSA-87 + Ed25519 signing material |
...
Each device generates a fresh hybrid ML-DSA-87 + Ed25519 signing keypair. The private signing key is protected locally alongside the RMS and is never sent to the server. The server stores the corresponding hybrid verification key.
...
4. Device A signs Device B's encapsulation key, signing verification key, and RMS capsule with its own enrolled hybrid identity signing key.
5. The server verifies Device A's single-use challenge signature and enrollment-payload signature before accepting the request. It stores the capsule and registers Device B's public key as authorized.
...
**First-device bootstrap:** On initial account creation (no prior enrolled device), the first device generates the RMS and its device identity keypairs. The server records this as the genesis device entry with no enroller authorization required.
...
- The `RMS` is split into a 2-of-3 scheme:
  ...
  - **Share 3:** Sealed into an authenticated, recipient-bound envelope (KEM to the contact's public key, context-bound AEAD) and handed to a trusted contact. The envelope is useless to anyone else and cannot be relabelled to another account, epoch, split, or coordinate. On a recovery request the contact re-seals the share to the requester's ephemeral key; raw share text never travels.
- Setup uses dedicated WebAuthn registration ceremonies: an authenticated device starts `/recovery/webauthn/register/start`, the client calls `navigator.credentials.create`, and `/recovery/webauthn/register/finish` verifies and stores the resulting passkey credential. Setup also stages a blind RMS-possession commitment (`possession_hash`) alongside Share 2; it is promoted atomically with the share at finalization.
- Recovery-share publication is two phase. Each client creates a UUID `split_id`, stages Share 2 with `PUT /recovery/share`, durably uploads an immutable Share 1 candidate carrying the same account, epoch, and split id, then calls `POST /recovery/share/finalize`. The server atomically promotes only the exact pending tuple; after success the client promotes that candidate to its mutable active cloud pointer. The first finalized split locks that epoch (except for exact idempotent retries); replacing it requires explicitly deleting the active recovery setup or rotating the RMS. A competing same-epoch setup therefore cannot mix the cloud share from one polynomial with the server share from another.
- Before the first WebAuthn, server, or cloud write, each client durably commits an encrypted recovery-publication journal containing the account, authenticated key epoch, split id, and exact shares. It commits `server_staged`, `cloud_candidate_durable`, `server_finalized`, and `cloud_active` only after each corresponding idempotent external operation succeeds. A restart resumes the same split; once server finalization is committed the setup cannot be discarded and must finish cloud promotion. A journal whose account or epoch is no longer active is retired before further external writes. Desktop uses an atomic, fsynced vault-encrypted file; Android uses Keystore-backed encrypted preferences with synchronous commits; Apple uses a device-only Keychain item.
- To recover, the user downloads the VELA app on a new device and reconstructs the `RMS` locally from **any two shares** (M18): cloud + server via `/recovery/initiate` + `/recovery/recover`, or — when either the cloud provider or the security key is lost — a trusted-contact pair. Contact-channel recovery starts `/recovery/initiate-proof`, the contact's app re-seals Share 3 to the requester's ephemeral key, and the client proves RMS possession (a challenge-bound keyed hash, verified against the staged commitment at `/recovery/recover/proof`). The possession path issues the same single-use grant without releasing Share 2 or asserting a credential. A hax-verified pair-selection policy enforces same account, epoch, split id, distinct channels, distinct coordinates, and recipient-bound contact envelopes before interpolation; rotation or setup deletion retires every old-epoch share including held contact copies.
- The recovered device still has no enrolled-device signature to authorize itself the normal way (§4.2), so it generates its own hybrid identity keypair locally and calls `/recovery/enroll-device` with the `recovery_grant` in place of an enrolling device's signature. The server verifies the grant is unexpired and unused, registers the new device against the existing account, and the grant is consumed — one successful recovery mints exactly one new device. The device then authenticates normally via `/auth/challenge` + `/auth/verify` and pulls the vault like any other device.
...
## 6. Hybrid Signature Authentication Protocol

The client authenticates by signing a server-issued nonce with its hybrid ML-DSA-87 + Ed25519 signing key. The server stores the corresponding hybrid verification key at registration or enrollment. Both signature components must verify.

### Flow
1. **Challenge:** Client requests a 32-byte challenge nonce from the server (stored in sled with a 60-second TTL; single-use).
2. **Signature Generation (Rust Core):** The client signs the length-prefixed, domain-separated transcript `vela auth challenge v1 || device_id || challenge`.
3. **Verification:** The client sends `(device_id, challenge, signature)` to the server. The Rust/Axum verifier consumes the challenge and checks both signature components against the registered hybrid verification key.
...
**Quantum-resistance advantage:** ML-DSA-87 is a NIST-standardized post-quantum signature scheme. Ed25519 is required alongside it as a classical defense-in-depth component. Captured challenge signatures cannot be replayed because challenges expire after 60 seconds and are consumed on first use.
...
### 7.1 Core Crypto Library (Rust)
- **Hybrid device signatures:** ML-DSA-87 + Ed25519 signatures are generated and verified in Rust.
- **Other cryptographic operations** (ML-KEM, X25519, XChaCha20-Poly1305, BLAKE3 KDF, Path ORAM, Shamir SSS) are implemented in Rust using audited crates (`ml-kem`, `x25519-dalek`, `chacha20poly1305`, `blake3`, etc.).
...
| :--- | :--- | :--- | :--- |
| `/device/enroll` | POST | Hybrid challenge signature (enrolling device) | Submits an RMS capsule encrypted for a new device's Hybrid PK, plus the enrolling device's signature over the enrollment payload. |
...
| `/recovery/share` | PUT/GET/DELETE | PASETO v4 | Stage, retrieve, or delete the user's encrypted server-side recovery share. New PUTs carry `key_epoch` and UUID `split_id`; GET returns the active share metadata. |
| `/recovery/share/finalize` | POST | PASETO v4 | Atomically promotes the exact pending `{key_epoch, split_id}` to active. Exact winner retries are idempotent; replaced or retired candidates conflict. |
...
| `/recovery/recover` | POST | WebAuthn assertion | Verifies the passkey assertion with user verification and releases the encrypted server-side recovery share plus a single-use `recovery_grant`. |
...
| VELA Server | Semi-trusted | Stores encrypted blobs; verifies hybrid signatures; never sees RMS or vault plaintext. The server is an honest-but-curious adversary: it cannot decrypt anything but can observe traffic timing. |
...
| Network | Untrusted | All communication over TLS 1.3; hybrid ML-DSA-87 + Ed25519 signatures provide authenticated challenge responses above TLS. |
...
| Quantum adversary breaking classical public-key cryptography | Hybrid ML-KEM + X25519 KEM; hybrid ML-DSA-87 + Ed25519 signatures for auth. |
