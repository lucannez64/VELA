...
impl EditableTextState {
    ...
    fn find_segment(&self, query: TextSegmentQuery) -> Result<(&TextLineSegment, usize), usize> {
        ...
        for segment in &self.layout_data.lines {
            let found = match &query {
                // VELA patch: must be inclusive of the segment's end. This is
                // called from `scroll_to_caret`, itself called synchronously
                // from the per-keystroke typing handler (`replace_text_in_
                // range`) BEFORE the next layout pass has re-shaped `self.
                // layout_data.lines` to include the just-typed character —
                // so right after typing the last character in a field, the
                // caret's new position is exactly `segment.text_range.end`
                // of the (still one-keystroke-stale) only/last segment. The
                // exclusive form here made that position match no segment
                // at all, silently falling through to `Point::default()`
                // (0, 0) and yanking the scroll/caret to the start of the
                // field on every keystroke that lands on the last character.
                TextSegmentQuery::CharacterPosition(pos) => segment.contains_position(*pos, true),
    ...
    fn find_point_for_character_position(&self, character_pos: usize) -> Point<Pixels> {
        ...
        let (segment, preceding_row_count) = match segment {
            Ok(found) => found,
            Err(_) => {
                // VELA patch: `character_pos` is beyond everything currently
                // shaped. This is the ordinary case right after typing new
                // character(s) at the end of a field — `scroll_to_caret`
                // runs synchronously from the per-keystroke handler, before
                // the next layout pass has re-shaped `layout_data.lines` to
                // include what was just typed, so the caret's brand-new
                // position has no segment yet. Falling back to the origin
                // here (as this used to) snapped the caret/scroll to the
                // start of the field on every keystroke that extended the
                // text past what had last been shaped — most visible once
                // the field needed to scroll (i.e. exactly when accuracy
                // here matters most). Approximate with the LAST known
                // segment instead: `TextLineSegment::position_for_index`
                // already clamps an out-of-range index to that segment's
                // own end, so this yields "the current known right edge of
                // the text" — correct one frame later once layout catches
                // up, and a much closer approximation in the meantime.
                let Some(last_segment) = self.layout_data.lines.last() else {
                    return Point::default();
                };
                let preceding_row_count = self.layout_data.lines[..self.layout_data.lines.len() - 1]
                    .iter()
                    .map(|s| s.row_count())
                    .sum();
                (last_segment, preceding_row_count)
            }
        };
...
impl EntityInputHandler for EditableTextState {
    ...
    fn replace_text_in_range(
        ...
        cx.emit(CaretNotify::PauseBlinking);
        // Upstream gap (VELA patch): unlike `move_to`/`select_to`, this is
        // the actual per-keystroke typing entry point (via
        // `EntityInputHandler`), and it never scrolled the caret into view —
        // so typing past a field's visible width left the tail invisible
        // until an arrow key or click (which do call `scroll_to_caret`)
    ...
    fn replace_and_mark_text_in_range(
        ...
        // VELA patch: same gap as `replace_text_in_range` above (IME
        // composition path never scrolled the caret into view either).
        self.scroll_to_caret();
        self.emit_text_changed(cx);
        cx.notify();
    }
