...
pub struct EditableTextElement {
    ...
    /// When set, the real stored text is substituted character-for-character
    /// with this char before shaping/painting (the placeholder is exempt —
    /// it's shown as-is, matching `<input type="password" placeholder=...>`).
    /// Caret positioning, selection, scrolling, and click-to-index all
    /// operate on this SAME substituted string (it's what's actually
    /// shaped), so they stay correct automatically — unlike substituting the
    /// display text via an external overlay element, which has no way to
...
impl EditableTextElement {
    ...
    /// Masks the displayed text character-for-character with `c` (the
    /// placeholder, if any, is shown as-is) — e.g. `text_input("pw").mask_char(Some('*'))`
    /// for a password field. `None` (the default) shows the real text.
    /// Caret/selection/scrolling/click-to-index all stay correct since the
    /// masked string is what's actually shaped and measured, not a separate
    /// overlay drawn on top — PROVIDED `c` is a single-byte (ASCII) char.
    /// Caret/selection positions are byte offsets into the *real* text,
    /// reused as-is as offsets into the masked shaped line; a multi-byte
    /// mask char (e.g. `'•'`, 3 bytes in UTF-8) desyncs that mapping against
    /// ordinary 1-byte-per-char ASCII text, visibly misplacing the caret.
    /// The same desync can still occur in the other direction if the real
    /// text itself contains multi-byte characters — there's no general fix
    /// for that without reworking this element's indexing to be char-based
    /// throughout, so this is a known limitation, not something `mask_char`
    ...
    fn request_layout(
        ...
        let (prelayout, next_scroll_offset) = {
            ...
            let text = match show_placeholder {
                false => Some(match self.mask_char {
                    Some(mask) => {
                        SharedString::from(mask.to_string().repeat(state.as_str().chars().count()))
                    }
                    None => SharedString::from(state.as_str()),
                }),
        ...
        window.request_measured_layout(
            ...
            move |known_dimensions, available_space, window, cx| {
                ...
                if let Some(size) = self.prev_layout_state.size
                    ...
                    && self.mask_char == self.prev_layout_state.last_seen_mask_char
                {
                    return size;
                }
                ...
                let layout_data = EditableTextLayoutResult {
                    ...
                    state: EditableTextLayoutState {
                        ...
                        last_seen_mask_char: self.mask_char,
                    },
                    lines,
                    line_height,
