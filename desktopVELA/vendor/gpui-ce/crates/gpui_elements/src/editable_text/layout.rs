...
pub(super) struct EditableTextLayoutState {
    ...
    /// The last seen `mask_char` (see `EditableTextElement::mask_char`) —
    /// toggling this changes the displayed/shaped text without touching
    /// `storage`'s own version counter, so it must be tracked separately or
    /// a reveal/hide toggle silently reuses stale (still-masked-or-not)
    /// cached lines.
    pub last_seen_mask_char: Option<char>,
}
