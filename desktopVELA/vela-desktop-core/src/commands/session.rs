//! Core of `src-tauri/src/commands/session.rs` — vault lifecycle (create,
//! unlock, lock, reset) and device/server registration. Toolkit-agnostic:
//! callers (Tauri command wrappers, gpui view actions) pass a plain
//! `&Arc<AppState>`, no `State<'_, T>`/`AppHandle`.
...
/// Locks the vault: clears session/crypto/in-memory vault state. Does not
...
pub async fn create_vault(state: &Arc<AppState>) -> Result<(), String> {
...
pub fn check_vault_exists(state: &Arc<AppState>) -> bool {
    state.store.has_existing_vault()
}
