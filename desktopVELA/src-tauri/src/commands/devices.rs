...
//
// Thin wrappers: the whole flow lives in `vela_desktop_core::commands::
// enrollment_v3` so the gpui build runs exactly the same code. In particular
// neither the correct fingerprint nor any private key is reachable from here —
// the frontend receives a shuffled list of candidates and answers by value.
//
// The v2 commands above stay registered alongside these. Enrollment codes are
// ephemeral, so nothing stored needs migrating, but installs mix: a v2 primary
// and a v3 joiner have to keep working until old builds age out.
