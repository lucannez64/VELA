...
async fn ensure_share_key(state: &AppState, client: &ApiClient, token: &str) {
    ...
    // Only the share keypair is being backfilled; everything else carries over
    // untouched, `hybrid_dk` included — a device that has one must not lose it
    // to a share-key backfill.
    let updated = vela_desktop_core::store::IdentityKeysStore {
...
async fn download_vault_from_manifest(
    ...
    for (idx, chunk_id) in ids.iter().enumerate() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            // foreign chunk); it must not abort the sync forever. It is
            // reported as `Corrupt` and the caller heals by re-uploading the
            // local vault. This branch's `ChunkOutcome` shape, kept over main's
            // `open_vault_chunk` helper (which is not in this tree).
            match decrypt(&key, &ciphertext) {
                Ok(chunk) => Ok::<_, String>(ChunkOutcome::Decrypted(idx, chunk.to_vec(), lamport)),
...
pub async fn trigger_sync(
    ...
    if let Err(e) = &downloaded {
        if is_auth_error(e) {
            // main's addition: refresh a stale auth token and retry the chunk
            // download once before giving up on the sync.
            match authenticate_for_sync(&state, &client, &device_id).await {
                Ok(new_token) => {
                    token = new_token;
